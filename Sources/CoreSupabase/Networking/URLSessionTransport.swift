...
public struct URLSessionTransport: ClientTransport {
  ...
  public func send(_ request: Request, baseURL: URL) async throws -> Response {
    let urlRequest = try URLRequest(request, baseURL: baseURL)
    let (responseBody, urlResponse) = try await invokeSession(urlRequest)
    return try Response(from: urlResponse, body: responseBody)
  }
...
internal enum URLSessionTransportError: Error {
  ...
  case invalidRequestURL(request: Request, baseURL: URL)
...
extension URLRequest {
  init(_ request: Request, baseURL: URL) throws {
    ...
    baseUrlComponents.queryItems = request.query.map {
...
extension URLSessionTransportError: CustomStringConvertible {
  public var description: String {
    ...
    case let .invalidRequestURL(request: request, baseURL: baseURL):
      return
        "Invalid request URL from request path: \(request.path), query: \(request.query) relative to base URL: \(baseURL.absoluteString)"
